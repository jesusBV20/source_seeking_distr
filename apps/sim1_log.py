...
SIMDATA_FILE = os.path.join(OUTPUT_DIR, "data_game.csv")
...
def setup_sim1(dt=0.01):
    ...
    simulator_engine = SimulationEngine(robot_model, controller, time_step=dt, log_filename=SIMDATA_FILE, log_time_step=0.1)

    fig, ax = plt.subplots(dpi=100, figsize=(16,9))
    simulator_plotter = PlotterUnySS(ax, simulator_engine.data)
