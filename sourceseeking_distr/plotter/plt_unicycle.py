...
- Unicycle simulation plotter class -
"""
...
class UnicyclePlotter:
    def __init__(
        self, simulator, team_tags=None, xlim=[-50, 80], ylim=[-50, 70], agents_size=3
    ):
    ...
    def plot_simulation(self, dpi=100):
        ...
        if self.team_tags is None:
            ...
            agents_colors = [
                TEAM_COLORS[self.team_tags[n]] if data_status[-1, n] else "red"
        ...
        # Agents icon
        for n in range(self.N):
            icon_i = unicycle_patch(
                data_p[0, n, :], data_phi[0, n], "royalblue", **self.kw_patch
            )
            icon_f = unicycle_patch(
                data_p[-1, n, :], data_phi[-1, n], agents_colors[n], **self.kw_patch
            )
    ...
    def anim_simulation(self, anim_tf=None, tail_frames=100, fps=60, dpi=100):
        ...
        for n in range(self.N):
            (line,) = ax.plot(
                data_p[0, n, 0], data_p[0, n, 1], c=agents_colors[n], zorder=1, lw=0.8
            )
        ...
        for n in range(self.N):
            icon = unicycle_patch(
                data_p[0, n, :], data_phi[0, n], agents_colors[n], **self.kw_patch
            )
            ax.add_patch(icon)
        ...
        def animate(i):
            ...
            for n in range(self.N):
                icons_list[n].remove()
                icons_list[n] = unicycle_patch(
                    data_p[i, n, :], data_phi[i, n], colors[n], **self.kw_patch
                )
